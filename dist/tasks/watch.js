...
module.exports = function (options) {
  return function () {
    ...
    var imagesWatcher = gulp.watch('./' + options.src + '/images/**/*.+(' + options.imageExtensions + ')');

    imagesWatcher.on('unlink', function (path) {
      options.deleteFile({
        path: path,
        event: 'unlink'
    ...
    gulp.watch(['./' + options.dest + '/**/*', './*.html']).on('change', options.browserSync.reload);
  };
};
